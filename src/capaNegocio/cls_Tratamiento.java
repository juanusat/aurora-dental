...
public class cls_Tratamiento {
    ...
    public int duracionTratamiento(int tratamiento_id) throws Exception {
        strSQL = "select duracion_estimada from tratamiento where tratamiento_id = " + tratamiento_id;
        try {
            rs = objBD.ConsultarBD(strSQL);
            if (rs.next()) {
                return rs.getInt("duracion_estimada");   
            }
        } catch (Exception e) {
            throw new Exception("Error al obtener duración de tratamiento: " + e.getMessage());
        }
        return 0;   
    }
